...
function checkAutoMerge({
  ...
  for (const rule of config) {
    ...
    if (
      ...
      !dependencyNames.includes(rule["dependency-name"])
    )
      matches = false;
    if (
      rule["package-ecosystem"] &&
      rule["package-ecosystem"] !== packageEcosystem
    )
      matches = false;
    ...
    matchedRule = rule;
    console.log(
      `Matched rule: ${JSON.stringify(rule)} for ${depNames} (${updateType}) -> ${shouldMerge}`,
    );
  }
